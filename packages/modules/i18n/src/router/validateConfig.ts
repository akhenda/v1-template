...
function validateConfig(config: Config): void {
  if (!Array.isArray(config.locales)) throw new Error(`The config requires a 'locales' array.`);
  if (!config.defaultLocale) throw new Error(`The config requires a 'defaultLocale'.`);
