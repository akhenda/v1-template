...
const OTHER_LINK_PROTOCOLS_REGEX = /^(\/|#|mailto:|sms:|fax:|tel:)/;
const DATA_BASE64_REGEX = /^data:[^;]+;base64,(.+)$/;
const DATA_BASE64_MIME_TYPE_REGEX = /data:([a-zA-Z0-9]+\/[a-zA-Z0-9-.+]+).*,.*/;
...
export const isUrl = (
  ...
  try {
    ...
    if (options.allowBase64 && url.protocol === 'data:') return IMAGE_DATA_REGEX.test(text);
...
export const sanitizeUrl = (
  ...
  }) || OTHER_LINK_PROTOCOLS_REGEX.test(url)
    ? url
    : `https://${url}`;
};
...
const base64MimeType = (encoded: string): string => {
  const result = encoded.match(DATA_BASE64_MIME_TYPE_REGEX);
  return result && result.length > 1 ? result[1] : 'unknown';
};

const isBase64 = (_str: string): boolean => {
  let str = _str;

  if (str.startsWith('data:')) {
    const matches = str.match(DATA_BASE64_REGEX);
