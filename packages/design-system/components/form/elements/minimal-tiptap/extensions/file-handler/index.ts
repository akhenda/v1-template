  ...
  return new Plugin({
    ...
    props: {
      handleDrop(view, event) {
        ...
        if (validFiles.length > 0 && onDrop) onDrop(editor, validFiles, pos?.pos ?? 0);
      },
      ...
      handlePaste(_, event) {
        ...
        if (errors.length > 0 && onValidationError) onValidationError(errors);
        if (validFiles.length > 0 && onPaste) onPaste(editor, validFiles, html);
      },
    },
  });
};
...
export const FileHandler = Extension.create<Omit<FileHandlePluginOptions, 'key' | 'editor'>>({
  ...
  addOptions() {
    return { allowBase64: false, allowedMimeTypes: [], maxFileSize: 0 };
  },

  addProseMirrorPlugins() {
    return [
      FileHandlePlugin({ key: new PluginKey(this.name), editor: this.editor, ...this.options }),
    ];
  },
});
