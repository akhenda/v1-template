...
type DownloadImageCommandProps = { src: string; alt?: string };
...
declare module '@tiptap/react' {
  interface Commands<ReturnType> {
    ...
    downloadImage: { downloadImage: (attrs: DownloadImageCommandProps) => ReturnType };
    copyImage: { copyImage: (attrs: DownloadImageCommandProps) => ReturnType };
    copyLink: { copyLink: (attrs: DownloadImageCommandProps) => ReturnType };
    toggleImage: { toggleImage: () => ReturnType };
  }
}
...
const handleImageUrl = async (src: string): Promise<{ blob: Blob; extension: string }> => {
  ...
  const extension = blob.type.split(BLOB_TYPE_SPLIT_REGEX)[1];
  return { blob, extension };
};

const fetchImageBlob = async (src: string): Promise<{ blob: Blob; extension: string }> => {
  return src.startsWith('data:') ? handleDataUrl(src) : await handleImageUrl(src);
};

const saveImage = (blob: Blob, name: string, extension: string): Promise<void> => {
