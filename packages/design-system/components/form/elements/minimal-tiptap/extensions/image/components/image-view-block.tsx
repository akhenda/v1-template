...
export const ImageViewBlock: React.FC<NodeViewProps> = ({
  ...
  const handleImageLoad = React.useCallback(
    (ev: React.SyntheticEvent<HTMLImageElement>) => {
      ...
      setImageState((prev) => ({ ...prev, naturalSize: newNaturalSize, imageLoaded: true }));
      updateAttributes({
      ...
      if (!initialWidth) updateDimensions((state) => ({ ...state, width: newNaturalSize.width }));
    },
  ...
  const handleResizeEnd = React.useCallback(() => setActiveResizeHandle(null), []);
  ...
  React.useEffect(() => {
    const handleImage = async () => {
      ...
      try {
        ...
        setImageState((prev) => ({ ...prev, ...normalizedData, isServerUploading: false }));
      ...
      >
        <div
          className={cn('relative flex h-full cursor-default flex-col items-center gap-2 rounded', {
            'outline-2 outline-primary outline-offset-1': selected || isResizing,
          })}
        >
            ...
            <div className="relative h-full">
              ...
              {imageState.error && (
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                  ...
                  <p className="mt-2 text-muted-foreground text-sm">Failed to load image</p>
                </div>
              )}
