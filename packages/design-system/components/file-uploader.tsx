...
/**
 * @see https://originui.com/search?tags=file
 */
...
const getFileIcon = (file: { file: File | { type: string; name: string } }) => {
  ...
  for (const { icon: Icon, conditions } of Object.values(iconMap)) {
    if (conditions(fileType, fileName)) return <Icon className="size-5 opacity-60" />;
  }
...
type UploadProgress = { fileId: string; progress: number; completed: boolean };
