  ...
  AIChatPlugin.configure({
    options: {
      promptTemplate: ({ isBlockSelecting, isSelecting }) => {
        if (isBlockSelecting) return PROMPT_TEMPLATES.userBlockSelecting;
        if (isSelecting) return PROMPT_TEMPLATES.userSelecting;

        return PROMPT_TEMPLATES.userDefault;
      },
      systemTemplate: ({ isBlockSelecting, isSelecting }) => {
        if (isBlockSelecting) return PROMPT_TEMPLATES.systemBlockSelecting;
        if (isSelecting) return PROMPT_TEMPLATES.systemSelecting;

        return PROMPT_TEMPLATES.systemDefault;
      },
    },
    render: { afterContainer: () => <AILoadingBar />, afterEditable: () => <AIMenu /> },
  }).extend({
          ...
          if (isFirst && mode === 'insert') {
            editor.tf.withoutSaving(() => {
              editor.tf.insertNodes(
                { children: [{ text: '' }], type: AIChatPlugin.key },
