...
export const getBlockType = (block: TElement) => {
  if (block[IndentListPlugin.key]) {
    if (block[IndentListPlugin.key] === ListStyleType.Decimal) return ListStyleType.Decimal;
    if (block[IndentListPlugin.key] === INDENT_LIST_KEYS.todo) return INDENT_LIST_KEYS.todo;
