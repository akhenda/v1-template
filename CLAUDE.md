...
bun run --filter app dev       # Dashboard only (localhost:3000)
bun run --filter web dev       # Marketing site only (localhost:3001)
bun run --filter api dev       # API server only (localhost:3002)
bun run --filter docs dev      # Documentation only (localhost:3004)
...
bun run --filter @repo/database db:studio  # Database GUI

# Convex Backend
bun run --filter @repo/backend dev     # Start Convex dev server
...
- Run `bun run --filter @repo/backend dev` to sync schema
